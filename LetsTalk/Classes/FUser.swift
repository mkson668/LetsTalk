...
// create an object to store in firebase database
...
class FUser {
    ...
    //MARK: Login function
    // completion is basically a callback where when everything is done
    
    class func loginUserWith(email: String, password: String, completion: @escaping (_ error: Error?) -> Void) {
        // this is the firebase login function, if success full it will return firebase user and and error
        Auth.auth().signIn(withEmail: email, password: password, completion: { (firUser, error) in
            
            if error != nil {
                // if some error occurs we just call our outside callback complwtion with our error
                completion(error)
                return
                
            } else {
                
                //get user information using the USERUID from firebase and save locally
